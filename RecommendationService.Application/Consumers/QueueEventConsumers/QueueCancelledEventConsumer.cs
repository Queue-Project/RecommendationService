...
namespace RecommendationService.Application.Consumers.QueueEventConsumers;

public class QueueCancelledEventConsumer: IConsumer<QueueEvent>
{
    private readonly ILogger<QueueCancelledEventConsumer> _logger;
    private readonly IRecommendationDbContext _dbContext;
    private readonly IRecommendationScoreService _scoreService;

    public QueueCancelledEventConsumer(
        ILogger<QueueCancelledEventConsumer> logger,
    ...
    public async Task Consume(ConsumeContext<QueueEvent> context)
    {
        ...
        if (request.Status != UpdatedQueueStatus.CanceledByCustomer &&
            request.Status != UpdatedQueueStatus.CanceledByEmployee &&
            request.Status != UpdatedQueueStatus.CanceledByAdmin)
        {
            return;
        }
        ...
        _logger.LogInformation(
            "Processing cancelled queue event. QueueId: {QueueId}, CompanyId: {CompanyId}, EmployeeId: {EmployeeId}",
            request.QueueId,
            request.CompanyId,
            request.EmployeeId);
        ...
        _logger.LogInformation(
            "Cancelled queue recommendation update finished. QueueId: {QueueId}",
            request.QueueId);
    }
}