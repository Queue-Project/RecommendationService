...
public class QueueCompletedEventConsumer : IConsumer<QueueEvent>
{
    private readonly ILogger<QueueCompletedEventConsumer> _logger;
    private readonly IRecommendationDbContext _dbContext;
    private readonly IRecommendationScoreService _scoreService;

    public QueueCompletedEventConsumer(
        ILogger<QueueCompletedEventConsumer> logger,
    ...
    public async Task Consume(ConsumeContext<QueueEvent> context)
    {
        ...
        if (request.Status != UpdatedQueueStatus.Completed)
        {
            return;
        }
        ...
        _logger.LogInformation(
            "Processing completed queue event. QueueId: {QueueId}, CompanyId: {CompanyId}, EmployeeId: {EmployeeId}",
            request.QueueId,
            request.CompanyId,
            request.EmployeeId);
        ...
        _logger.LogInformation(
            "Completed queue recommendation update finished. QueueId: {QueueId}",
            request.QueueId);
    }
}